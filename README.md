...
插件按需读取 Claude 自己保存在 `~/.claude/projects/**/<session-id>.jsonl` 的 transcript，不复制聊天内容，也不索引 subagent/sidechain、thinking、工具参数或工具输出。subagent hook 不会更新状态圆点或覆盖主会话映射。没有活动 pane 映射的旧会话不会显示，因为它们不存在可可靠跳转的 window。已使用旧版状态 hook 的 Claude 会在下一次状态事件时建立映射；首次安装 hooks 后，已经运行的 Claude Code 仍需重启一次。
